...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border-transparent bg-gradient-to-r from-primary to-accent text-primary-foreground", // Updated to use primary/accent gradient
        destructive:
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ...
    className={cn(
      ...
      // Ensure action button text is visible on new gradient default toast
      "group-[&:not(.destructive)]:border-primary-foreground/40 group-[&:not(.destructive)]:hover:bg-primary-foreground/10 group-[&:not(.destructive)]:text-primary-foreground group-[&:not(.destructive)]:focus:ring-primary-foreground",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ...
    className={cn(
      ...
      // Default variant close button styles
      "text-primary-foreground/80 hover:text-primary-foreground",
      // Destructive variant close button styles
